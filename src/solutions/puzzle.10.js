...
export function solvePart1() {
    const map = getMapMatrix(input);

    const trailHeads = [];
    let sumOfTrailTops = 0;
    ...
    for (let i = 0; i < trailHeads.length; i++) {
        ...
        let height = 0;
        let nextStep = { row: trailHead.i, col: trailHead.j, value: height };
        let path = [nextStep];
        let blackList = [];
        let headTrailTops = [];
        ...
        do {
            ...
            if (steps?.length > 1) {
                path[path.length - 1].XR = true;
            }

            if (steps?.length > 1) {
                nextStep = steps.find(step => !blackList.some(black => black.row === step.row && black.col === step.col));
            } else {
            ...
            if (!nextStep) {
                ...
                if (pathHasCrossRoad) {
                    ...
                    addTrailTop(headTrailTops, height, path[path.length - 1]);

                    // reset trail
                    height = 0;
                    nextStep = { row: trailHead.i, col: trailHead.j, value: height };
                    path = [nextStep];
                } else {
                    addTrailTop(headTrailTops, height, path[path.length - 1]);
                }
        ...
        sumOfTrailTops += headTrailTops.length;
    }

    return sumOfTrailTops;
}

// 1269 too low
export function solvePart2() {
    const map = getMapMatrix(testInput4);
        ...
        do {
            ...
            if (!nextStep) {
                ...
                if (pathHasCrossRoad) {
                    ...
                    const lastCrossRoadIndex = path.indexOf(lastCrossRoad);
                    blackList.push(path[lastCrossRoadIndex + 1]);
                    addTrailTop(headTrailTops, height, path[path.length - 1], true);
...
function addTrailTop(trailTops, height, top, addDuplicate = false) {
    if (height - 1 == TOP && (addDuplicate || !trailTops.find(x => x.row == top.row && x.col == top.col))) {
        trailTops.push(top);
        console.debug('trailTops', trailTops);
    }
}

function findNextStep(map, row, col, nextHeight, blackList) {
    const steps = [
        { row: -1, col: 0 },
        { row: 1, col: 0 },
        { row: 0, col: -1 },
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        if (map[nextRow] && +map[nextRow][nextCol] === (+nextHeight)) {
            if (!blackList.some(step => step.row === nextRow && step.col === nextCol)) {
                possibleSteps.push({ row: nextRow, col: nextCol, value: +nextHeight });
            }
        }
    }
...
function getMapMatrix(inp) {
    return inp.split(LINE_SEP).map(line => line.split(''));
}

function drawMap(path, map) {
    const mapCopy = map.map(row => [...row]);
    path.forEach(step => {
        mapCopy[step.row][step.col] = '#';
    });
    console.debug(mapCopy.map(row => row.join('')).join(LINE_SEP));
